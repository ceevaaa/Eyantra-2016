...
def detectCellVal(img_gray,grid_map):
	#your code here
	templates = cv2.imread('digits/1.jpg') # templates is a variable to store the template which is to be compared in the task image
	...
	gray_template = cv2.cvtColor(templates, cv2.COLOR_BGR2GRAY) #gray_template is to store the gray image of the given template
    # so that it can be compared in the task image

	result = cv2.matchTemplate(img_gray, gray_template, cv2.TM_CCOEFF_NORMED) #result is a numpy.nd array which stores the extent of template matching
    #Expected range : no

	threshold = 0.40 #threshold is a value above which the extent of matching is to be considered
	locations = np.where(result >= threshold) #locations is list to store the points where the result is greater than threshold
	for point in zip(*locations[::-1]): #point stores the x and y coordinates of the point where the template is matched
		if point[0] % 50 == 0 and point[1] % 50 == 0: #point[0] is the x coordinate and point[1] is the y coordinate of the matched point
...
def around(row, col, visited, grid_map_2):
        Available_ones = []  # Available_ones is a list which stores the coordinates of the available cells having value '1' around the given cell grid_map_2[r][c]  and were not visited earlier
...
def solveGrid(grid_map):
    ...
    for i in range(0, len(starting_points)):
        ...
        # a matrix which contains 2 more rows and 2 more columns than grid_map, usage is defined below
        # Each element row,col of grid_map will be row+1,col+1 of grid_map_2, which will be assigned later
